...
export class ProjectWorkspace {
  ...
  async open() {
    ...
    if (!this.loaded) {
      ...
      this.loaded = true;
      const owner = { member: 'MemberCard', admin: 'AdminCard', toolbar: 'AccessToolbar', parent: 'TeamAccess' }[this.view.selected];
  ...
  syncSkin() {
    ...
    const path = `${PREFIX}ProfileCard/index.css`;
  ...
  async run() {
    ...
    try {
      ...
      const entryDocument = new DOMParser().parseFromString(entry, 'text/html');
      const mapNode = entryDocument.querySelector('script[type="importmap"]');
      ...
      mapNode.textContent = JSON.stringify(map);
      ...
      await cache.put(new URL('index.html', base), new Response(entryHTML, { headers: { 'Content-Type': 'text/html' } }));
