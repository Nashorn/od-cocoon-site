...
class Showcase extends Component {
  ...
  async onConnected() {
    ...
    this.bindSelection();
    this.listen(this.$('#try-flow'), 'click', () => this.workspace.open());
  ...
  async selectTab(tab) {
    ...
    const name = { profile: 'BaseProfileCard', member: 'MemberCard', admin: 'AdminCard', toolbar: 'AccessToolbar', parent: 'TeamAccess' }[this.selected];
    this.$('#ancestry').textContent = this.selected === 'profile' ? 'MemberCard and AdminCard share this base class' : inherited ? 'extends BaseProfileCard' : this.selected === 'parent' ? 'assembles three child components' : 'Fictitous Toolbar';
    ...
    const editable = tab === 'css' && (inherited || this.selected === 'profile');
    this.$('#css-editor').hidden = !editable; this.$('#source-code').hidden = editable; this.$('#reset-style').hidden = !editable;
    const owner = inherited && tab !== 'js' ? 'BaseProfileCard' : name;
  ...
  styleResult() { this.workspace?.syncSkin(); this.$('#inspector-result').classList.remove('error'); this.$('#inspector-result').textContent = 'Updated BaseProfileCard → MemberCard + AdminCard. Both use the same inherited stylesheet.'; }
